...
$data = [];
if ($empId) {
    $stmt = $pdo->prepare("SELECT m.*, CONCAT(s.first_name,' ',s.last_name) as sender_name FROM messages m JOIN users s ON m.from_user_id = s.id WHERE m.to_user_id = ? ORDER BY m.created_at DESC LIMIT 30");
...
<div class="page-header"><div><h1 class="page-title">Track Complaint</h1><p class="page-subtitle mb-0">Incoming requests and complaints</p></div></div>
<div class="px-4 pb-4">
...
<?php endforeach; if(empty($data)): ?><tr><td colspan="4" class="text-center text-muted py-5">No complaints to track</td></tr><?php endif; ?>
