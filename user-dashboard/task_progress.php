...
<p class="page-subtitle mb-0">Your assigned tasks</p>
</div>
</div>
<div class="px-4 pb-4">
...
<div class="progress-bar" style="width:<?php echo $row['progress']; ?>%;background:var(--primary);"></div>
</div>
<?php echo $row['progress']; ?>%
</td>
<td><?php echo date('M j', strtotime($row['due_date'])); ?></td>
<td><span class="status-badge <?php echo $sc; ?>"><?php echo ucfirst($row['status']); ?></span></td>
</tr>
<?php endforeach; if(empty($data)): ?>
<tr><td colspan="5" class="text-center text-muted py-5">No tasks</td></tr>
<?php endif; ?>
</tbody>
</table>
</div>
